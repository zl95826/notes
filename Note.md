...
window.getComputedStyle(element [, pseudoElt]); The .style object only includes style properties directly set on the DOM element, which means inline style. Properties determined from CSS are not visible that way.// The object from getComputedStyle is read-only.\
