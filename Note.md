...
setTimeout(func); // using setTimeout we can execute code after the stack is empty \
...
window.getComputedStyle(element [, pseudoElt]);<b> The .style object only includes style properties directly set on the DOM element, which means inline style. Properties determined from CSS are not visible that way.</b>// The object from getComputedStyle is read-only.\
...
It returns <i>this</i> automatically if the function doesn't return a different (its own) object, so that the object that is created is returned from the function.https://javascript.info/constructor-new\
