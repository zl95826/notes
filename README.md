...
Math.floor(x) returns the value of x rounded down to its nearest integer; Math.ceil(x) returns the value of x rounded up to its nearest greater intefer; Math.round(x) returns the value of x rounded to its nearest greater integer.四舍五入\
